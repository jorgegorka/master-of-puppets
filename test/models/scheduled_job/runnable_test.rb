...
class ScheduledJob::RunnableTest < ActiveSupport::TestCase
  ...
  test "run_now produces a succeeded JobRun with cost + output captured" do
  ...
  test "run_now advances next_run_at + last_run_at" do
    adapter = LlmStubs::StubAdapter.new(text: "ok")
    before  = @sj.next_run_at

    with_stubbed_llm(adapter) { @sj.run_now }
    @sj.reload
  ...
  test "adapter raises → JobRun is :failed with error_message" do
    ...
    run = nil
    with_stubbed_llm(adapter) { run = @sj.run_now }

    assert run.failed?, "expected run to be failed, was #{run.status}"
    assert_equal "boom", run.error_message
  end

  test "run_now tracks scheduled_job_run_started and scheduled_job_run_succeeded events" do
  ...
  test "run_now truncates output that exceeds MAX_OUTPUT_BYTES" do
  ...
  test "run_now records original byte size when output is truncated" do
    big = "x" * (ScheduledJob::Runnable::MAX_OUTPUT_BYTES + 1024)
    adapter = LlmStubs::StubAdapter.new(text: big)
  ...
  test "run_now leaves output_truncated_at_bytes nil when output fits" do
