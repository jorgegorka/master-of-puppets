...
This documentation explains the patterns, conventions, and best practices used in the Talento HQ Rails application. It's designed for developers who already know Ruby on Rails and need to understand how Talento HQ structures and organizes code.

We want to apply these patterns in TalentoHQ when possible.

## Table of Contents

- [Part 1: Foundation & Architecture](#part-1-foundation--architecture)
  - [1.1 Understanding Talento HQ's Architecture](#11-understanding-fizzys-architecture)
...
- [Part 5: Coding Style Guide](#part-5-coding-style-guide)
  - [5.1 Talento HQ Coding Conventions](#51-fizzy-coding-conventions)
...
This is where Talento HQ's distinctive style shines. Models contain business logic and use concerns heavily to organize behavior.

Please read and understand @docs/guides/active-record.md to better decide which kind of models we need to create.

## 2.1 Concern Architecture

Talento HQ uses concerns extensively to compose model behavior. This is the most distinctive pattern in the codebase.
...
4. Private methods (if needed) - Indented per Talento HQ style guide
...
Talento HQ emphasizes method names that read like business domain language. Models provide APIs that express intent clearly.
...
Talento HQ uses lambda defaults on `belongs_to` associations to automatically propagate context. This reduces boilerplate and enforces multi-tenancy.
...
Scopes in Talento HQ have descriptive names that express business concepts, not SQL operations. They're composable and chainable.
...
Talento HQ uses callbacks sparingly. The philosophy: explicit is better than implicit.

### Minimal Callback Philosophy
...
Talento HQ has several unique domain-specific features that follow consistent patterns.

## 3.1 Event Tracking System

Events are Talento HQ's audit trail. Every significant action creates an event record that drives activity timelines, notifications, and webhooks.
...
Talento HQ tracks file storage usage at the board and account level to enforce quotas. This pattern shows how to handle complex accounting across hierarchies.
...
Talento HQ's unique "entropy" feature automatically postpones stale cards to prevent infinite todo lists. This shows how to implement complex time-based business rules.
...
Controllers and background jobs in Talento HQ are extremely thin. They orchestrate, but delegate all business logic to models.

## 4.1 Thin Controllers with Rich Models

Talento HQ strictly follows the "thin controller, fat model" philosophy. Controllers have 3 responsibilities: setup, call model, respond.
...
**✅ Talento HQ pattern (model handles logic):**
...
Talento HQ models all actions as RESTful resources, not custom routes. This is a core pattern from the style guide.
...
Talento HQ has a strict naming convention for asynchronous operations. Jobs are ultra-thin and delegate everything to models.
...
Multi-tenancy "just works" in jobs without thinking about it. This is one of Talento HQ's most powerful patterns.

---

# Part 5: Coding Style Guide

Talento HQ has specific code style conventions beyond standard Ruby/Rails style.

## 5.1 Talento HQ Coding Conventions

### Conditional Returns (Expanded Conditionals Preferred)

Talento HQ prefers expanded conditionals over guard clauses:
...
Step-by-step guides for common development tasks in Talento HQ.
...
```ruby
resources :cards do
  resource :closure  # ← Talento HQ pattern
end
```
...
This documentation covers the core patterns and practices used throughout the Talento HQ Rails application:
...
- **Style**: Talento HQ-specific conventions for readable code
